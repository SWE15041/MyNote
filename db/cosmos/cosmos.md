...
- 根据创建容器时设置的预配吞吐量和 总存储容量，创建物理分区由系统内部实现，完全由cosmos DB 管理。
  
- 容器的物理分区数量跟以下的配置有关：
  - 预配吞吐量. (每个单独的物理分区 每秒最多可以处理10000个RU.)
  - 数据总存储容量. (每个单独的物理分区最大可以存放50G的数据)
- 关键知识点：
  - 逻辑分区和物理分区的关系：一个或者多个逻辑分区可以映射到一个物理分区（物理分区：逻辑分区 = 1：n）
  - 无数量限制：容器中的物理分区总数没有限制。
  - 自动扩建：当容器的预配置吞吐量或者总存储容量增长的时候，会自动创建新的物理分区，会拆分旧物理分区，并建立新的物理分区到 逻辑分区 的映射。
  - **单个逻辑分区**的数据存放在**同一块**物理分区内。
  - “热分区”：容器的预配吞吐量由被**平均分配**到每个物理分区。如果分区键设计不合理，将导致大量的请求被定向到某个逻辑分区内，从而形成“热分区”，会导致请求频率受限和高花费。
...
- 关键知识点：
  ...
  - 分区键设置后不可更改：选择分区键后，将无法就地更改它。（如果需要更改分区键，则应使用所需的新分区键将数据移动到新容器中。）
...
- 分区键的选择
  ...
  - 大多数情况下如何选择分区键：
    ...
    ```
    - 分区键值不可变：项一旦创建，则相应的分区键值不可变。（如果项初始化的时候没有给分区键赋值，那么后续也做不了赋值操作）
    - 分区键的键值在容器中能够被广泛使用
    - 逻辑分区平均消费RU可以确保物理分区的RU被平均消费。
    ```

  - 重读取型容器如何选择分区键：

    ```
    - 使用查询时需要经常用到的项属性作为分区键，在查询的时候把分区键作为谓词进行查询，能够快速路由到相应 物理分区。
    - 可以减少跨分区查询的项 可以被设置为分区键。
    - 经常被用作查询的项可以被设置为分区键。
    ```
